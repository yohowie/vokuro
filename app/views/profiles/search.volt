...
<div class="btn-group mb-5" role="group">
    ...
    {{ link_to('profiles/create', '创建 Profiles', 'class': 'btn btn-primary') }}
</div>
    ...
    <tbody>
        ...
        <tr>
            ...
            <td class="td-width-12">
                {{ link_to('profiles/edit/' ~ profile['id'], '<i class="icon-pencil"></i> 编辑', 'class': 'btn btn-sm btn-outline-warning') }}
            </td>
            <td class="td-width-12">
                {{ link_to('profiles/delete/' ~ profile['id'], '<i class="icon-remove"></i> 删除', 'class': 'btn btn-sm btn-outline-danger') }}
            </td>
        </tr>
        {% else %}
        <tr>
            <td colspan="10" class="text-center">没有记录任何配置文件</td>
        </tr>
        {% endfor %}
    </tbody>
    <tfoot>
